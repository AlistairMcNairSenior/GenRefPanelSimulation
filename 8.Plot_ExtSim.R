...
# Read in the data
files<-dir("ExtSim")[-1001]
data<-read.csv(paste0("ExtSim/", files[1]))
for(i in 2:length(files)){
	data<-rbind(data, read.csv(paste0("ExtSim/", files[i])))	
}
dim(data)
...
pdf("ExtSim.pdf", height=4, width=4)
...
for(g in 1:length(sigma_G)){
	...
	p1<-ggplot(G1, aes(x=n-0.3, y= m.np_md_CV)) +
		...
		scale_y_continuous(lim = c(-1, 4),  sec.axis=sec_axis(trans= ~./4, name = "Replicability", breaks=seq(-1, 1, 0.2))) +
